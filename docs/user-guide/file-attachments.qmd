...
**Note**: For interactive multi-turn conversations with files, use the browser interface with `bot.show("browser")` instead.
...
Perfect for automated code quality checks and pull request reviews:
...
def analyze_visual_content(image_paths, analysis_focus="general"):
    ...
    analyzer = (tb.ChatBot()
                .provider_model("openai:gpt-4-turbo")  # Has vision capabilities
                .temperature(0.5))
...
**File Not Found**
```
FileNotFoundError: [Errno 2] No such file or directory: 'missing.pdf'
```
...
```
Content type 'unsupported' for file: document.docx
```
- Convert files to supported formats (PDF, common image formats, text)
- Check the supported file types table above

**Large File Processing**
- Break large files into smaller chunks
- Use text extraction tools for very large PDFs
- Consider summarizing content first

**Model Limitations**
- Some models don't support image analysis
- Use `gpt-4-turbo` or `gpt-4-vision-preview` for images
- Check model capabilities for your use case
...
3. Review the metadata for error details
4. Test with smaller files first
5. Check model capabilities match your needs

## Integration Examples

### With Jupyter Notebooks

File attachments provide rich HTML displays in Jupyter notebooks, making it easy to see file status, sizes, and prompts at a glance.
...
# Rich HTML display shows file summary
...
result  # Rich HTML display in Jupyter
```
...
def main():
    if len(sys.argv) < 2:
        print("Usage: review.py <file1> [file2] ...")
        sys.exit(1)

    files = tb.Attachments(*sys.argv[1:]).with_prompt(
    ...
    review = reviewer.chat(files)

    # Save review to file
    output = Path("code_review.md")
    output.write_text(str(review))
