...
tech_bot = (tb.ChatBot()
    .model("gpt-4o")           # Choose the AI model
...
```python
# OpenAI models
bot = tb.ChatBot().model("gpt-4o")      # Best reasoning
bot = tb.ChatBot().model("gpt-4o-mini")    # Fast and cost-effective

# Anthropic models
bot = tb.ChatBot().model("claude-sonnet-4-20250514")    # Excellent for creative tasks
bot = tb.ChatBot().model("claude-haiku-3.5-20241022")   # Fast for simple tasks
...
::: {.callout-tip}
## Quick Start with Persona Packs

For an even faster setup, use `.persona_pack()` to load a pre-built persona with optimized prompts, vocabulary, and behavior:

```python
# One-line expert setup
bot = tb.ChatBot().persona_pack("code_reviewer")
```

Persona packs bundle a `PromptBuilder` configuration, vocabulary terms, and recommended settings into a single method call. See the Personas documentation for available packs.
:::
...
class SmartChatBot:
    def __init__(self):
        self.bot = tb.ChatBot().model("gpt-4o")
...
- Try a simpler model like `gpt-4o-mini`
...
**Basic Setup**: `tb.ChatBot().model("gpt-4o").preset("technical_advisor")`
