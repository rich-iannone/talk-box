...
- **`number_sequence`**: generate numeric sequences with start, stop, and step
...
- **`to_json`**: convert data structures to JSON strings
- **`sort_list`**: sort lists of values
...
- **`parse_url`**: parse URLs into their components
- **`url_encode_decode`**: URL encode/decode strings
- **`path_info`**: extract information from file paths
